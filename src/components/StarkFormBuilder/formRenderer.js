...
export default function FormRenderer(props) {
  ...
  const updateFormValues = (e, field, fieldIndex = 0, aField = {}) => {
    ...
    if (field.type === "addmore") {
      ...
      fieldValues[fieldIndex][aField.name] = allValues[field.name] =
        fieldValues;
    } else {
    ...
    if (field.type === "text" && field.maxNumber > 0) {
  ...
  const checkFieldCondition = (condition) => {
    ...
    switch (condition.condition) {
      ...
      case "==":
        ...
        if (fieldType.type === "text" && fieldType.maxNumber > 0) {
          conditionResults = condition.value
            .split(",")
            .includes(formValues[condition.name]);
